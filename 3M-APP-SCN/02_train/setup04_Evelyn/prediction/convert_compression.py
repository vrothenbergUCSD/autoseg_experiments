...
# Initialize logging
logging.basicConfig(filename='convert_compression.log', 
                    level=logging.INFO, 
                    format='%(asctime)s - %(levelname)s - %(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S')

def convert_compression(args):
  ...
  try:
    for layer_idx in range(start_layer, end_layer):
      ...
      dest_array[layer_idx, :, :] = layer
      progress_bar.value += 1
  except Exception as e:
    logging.error("Exception occurred", exc_info=True)

# Open source Zarr array
source_group = zarr.open('3M-APP-SCN_remapped_32bit.zarr', mode='r')
source_array = source_group['segmentation_0.1']

# Create destination Zarr array with Gzip compression
compressor = zarr.GZip(level=5)
dest_group = zarr.open('3M-APP-SCN_remapped_32bit_gzip.zarr', mode='a')
dest_array = dest_group.create_dataset('segmentation_0.1', shape=source_array.shape, chunks=source_array.chunks, dtype=source_array.dtype, compressor=compressor)
...
with Manager() as manager:
  ...
  try:
    with Pool(num_cores) as pool:
      async_result = pool.map_async(convert_compression, args)
...
logging.info('Completed compression conversion of 3M-APP-SCN_remapped_32bit.zarr')
