...
data_path = "/data/base/3M-APP-SCN/01_data/APP-3M-SCN-3000.zarr"
...
def calc_max_padding(
    ...
    # Add logging to TensorBoard separately
class TensorBoardLogging(gp.BatchFilter):
    def __init__(self, summary_writer, every=100):
    ...
    def process(self, batch, request):
        ...
        if self.iteration % self.every == 0:
            loss = batch.loss
            self.summary_writer.add_scalar('loss', loss, self.iteration)
...
def pipeline(iteration):
    ...
    pipeline += TensorBoardLogging(writer)

    with gp.build(pipeline):
        for i in range(iteration):
            pipeline.request_batch(request)
            if i % 100 == 0:
                logging.info(f"Completed iteration {i}/{iteration}")
